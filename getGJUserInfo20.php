...
if($target == $accountID) {
    ...
    $data = $query->fetch();
    //:31:FRIEND_STATE:44:
    echo '1:'.$data['userName'].':2:'.$data['ID'].':13:'.$data['coins'].':17:'.$data['userCoins'].':10:'.$data['color1'].':11:'.$data['color2'].':3:'.$data['stars'].':46:'.$data['diamonds'].':4:'.$data['demons'].':8:'.$data['cp'].':18:'.$data['ms'].':19:'.$data['frs'].':50:'.$data['cs'].':20:'.$data['yt'].':21:'.$data['accIcon'].':22:'.$data['accShip'].':23:'.$data['accBall'].':24:'.$data['accBird'].':25:'.$data['accDart'].':26:'.$data['accRobot'].':28:'.$data['accGlow'].':43:'.$data['accSpider'].':47:'.$data['accExplosion'].':30:1:16:'.$data['ID'].':31:1:44:'.$data['twitter'].':45:'.$data['twitch'].':29:1:49:1';
}